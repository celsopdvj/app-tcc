...
class FormularioDeEnvioTCC extends StatefulWidget {
  final User user;
  final User aluno;
  FormularioDeEnvioTCC({Key key, @required this.user, @required this.aluno})
      : super(key: key);
  @override
...
class FormularioDeEnvioTCCState extends State<FormularioDeEnvioTCC> {
  ...
  Widget build(BuildContext context) {
    Future<void> _uploadFile(File file, String filename) async {
      ...
      print("URL is $url");
      await banco.salvarTCC(widget.aluno.uid,nomeAluno, titulo, area, url, filename);
    }
      ...
      Future filePicker(BuildContext context) async {
        try {
          ...
          showDialog(
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  title: Text('Erro ao enviar TCC ou nenhum documento foi escolhido.'),
                  actions: <Widget>[
                    FlatButton(
                      textColor: Colors.white,
                      color: Colors.blue[300],
